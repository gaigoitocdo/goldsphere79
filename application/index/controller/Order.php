...
class Order extends Base
{
	...
	public function addorder()
	{
		...
		*/
		...
		if($data['order_price'] < $conf['order_min_price'] || $data['order_price'] > $conf['order_max_price']){
			...
			return WPreturn('Xin lỗi! Có lỗi không xác định!', -1);
		}
		//手续费
        ...
        if($ids){
        	...
        	set_price_log($data['uid'], 2, $u_fee, 'Đặt hàng', 'Đặt hàng thành công', $ids, $nowmoney);
	...
	public function orderlist()
	{
		...
		$map = array('uid'=>$uid,'ostaus'=>0);
		$map['selltime'] = array('lt',time());

    $holdsave = Db::name('order')->where($map)->order('oid desc')->select();

    if ($holdsave) {
        $updateData = array('ostaus' => 1);
        //Db::name('order')->where($map)->update($updateData);
    } 
	...
	public function goorder()
	{
		...
		if(isset($order['sellprice']) && $order['sellprice'] != $price){
			...
			if( $price == $order['buyprice'] && $order['is_win'] != 3){
				...
				db('userinfo')->where('uid',$order['uid'])->setDec('usermoney',$u_add);db('price_log')->where('id',$price_log['id'])->delete();
               	//写入日志
	           	$api = controller('api');
    ...
    public function lixibao_in()
    {
        ...
        set_price_log($uid,2,$money,'Tiền gửi tiết kiệm', 'Chuyển tiền vào tiền gửi tiết kiệm',0,$newprice);
        set_price_log($uid,1,$money,'Tiền gửi tiết kiệm', 'Chuyển tiền vào tiền gửi tiết kiệm',0,$noemoney);
    ...
    public function lixibao_out()
    {
        ...
        Db::name('userinfo')->where('uid',$uid)->update(array('usermoney'=>$newprice));
        Db::name('userinfo')->where('uid',$uid)->update(array('lixibao'=>$noemoney));
        set_price_log($uid,1,$money,'Tiền gửi tiết kiệm', 'Chuyển tiền từ tiền gửi tiết kiệm',0,$newprice);
        set_price_log($uid,2,$money,'Tiền gửi tiết kiệm', 'Chuyển tiền từ tiền gửi tiết kiệm',0,$noemoney);
        return WPreturn('Succeful！',1);
    }
    ...
    public function mujuan()
    {
        ...
        $zjpwd = $post['zjpwd'];
        if(!$money){
            return WPreturn('Vui lòng nhập số tiền hợp lệ!',-1);
        }
        if(!$zjpwd){
            return WPreturn('Vui lòng nhập mật khẩu tài khoản!',-1);
        }
        $uid = $this->uid;;
        $user = Db::name('userinfo')->where('uid',$uid)->find();
        if($user['usermoney'] < $money){
        ...
        if($user['paypwd'] != $zjpwd){
            return WPreturn('Mật khẩu không đúng!',-1);
        }
        $newprice = $user['usermoney']-$money;
        //$noemoney = $user['lixibao']+$money;
        Db::name('userinfo')->where('uid',$uid)->update(array('usermoney'=>$newprice));
        set_price_log($uid,2,$money,'Tiền quỹ','Chuyển tiền quỹ ra bên ngoài',0,$newprice);

        return WPreturn('Cảm ơn bạn đã đóng góp！',1);
    }
