...
class Api extends Controller
{
	 ...
	 * @param  [type] $userinfo Tên đăng nhập
	...
	public function daifu($balance,$userinfo,$bank)
	{
		...
		if($balance['isverified'] != 0){
			return WPreturn('Lệnh này đã được xử lý!!',-1);
		}
		...
		$data['liqType'] = 'T0';
		$data['openBank'] = $bank['bank_nm'];
		$data['voucher'] = "Thẻ căn cước";
		$data['voucherId'] = $bank['scard'];
		$data['reqUrl'] = "http://".$_SERVER['SERVER_NAME']."/lord/api/daifurefund.html";
		...
		$url="http://pay.goldepay.cn00/mobile/wdl/payWith.do";
	...
	public function daifurefund()
	{
		...
		$userOrder 	= $_POST['userOrder']; 		 //接收传递的Số Lệnh
