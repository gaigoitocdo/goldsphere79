...
class Pay extends Controller
{
    ...
    public function wxpay($data)
    {
        ...
        if (!empty($data)) {
            ...
            if(!$openId){
                return WPreturn('openId không tồn tại',-1);
            }
            //统一下单
            $input = new WxPayUnifiedOrder();
            $input->setBody("Nạp tiền thành viên");
    ...
    $pay_orderid = $data['balance_sn'];    //Số Lệnh
    $pay_amount = $data['bpprice'];    //交易金额
    $pay_applydate = date("Y-m-d H:i:s");  //Thời gian đặt lệnh
	if($type == 'zypay_wx'){
    ...
    $tjurl = "http://zy.cnzypay.com0/Pay_Index.html";   //提交地址,如有变动请到官网下载最新接口文档
    ...
    $requestarray = array(
		...
		if(isset($arr[1])){
			$arr2 = explode('"',$arr[1]);
			if(isset($arr2[0])){
				return "http://zys.cnzypay.com0/".$arr2[0];
			}
		}
        ...
        $str = $str . '<input type="submit" value="Gửi">';
        $str = $str . '</form>';
        $str = $str . '<script>';
    ...
    public function zypay_notify()
    {
        ...
        $ReturnArray = array( // 返回字段
            ...
            "orderid" =>  $_REQUEST["orderid"], // Số Lệnh
    ...
    public function cardpay(){
		...
		$callback_url		= "http://am.bfdee.cn0/index/pay/cardpay";   
		$callback_url_muti  = "http://am.bfdee.cn0";
		...
		//set_price_log($balance['uid'],1,$balance['bpprice'],'Nạp tiền','Người dùng nạp tiền',$_edit['bpid'],$_edit['bpbalance']);
		
		set_price_log($balance['uid'],1,$balance['bpprice'],'Nạp tiền','Người dùng nạp tiền',$balance['bpid'],$_edit['bpbalance']);
    ...
    public function qianbaotong($data,$pay_type,$type=0)
    {
        ...
        */
        ...
        /*
        * 请求发起方自己的Số Lệnh，该Số Lệnh将作为平台的返回数据
        */
        ...
        */
        ...
        $shidai_bank_url   = 'http://newpays.qpabc.com0/bank/';
		...
		$curl = curl_init();
			...
			if(in_array($pay_type,array(1003,1004,1009))){
				$res_arr = json_decode($res,1);

				if(isset($res_arr["retCode"]) && $res_arr["retCode"] == '0000' && $res_arr["tranStatus"] == 'success'){
					
					return $res_arr["codeUrl"];
				}else{
					return false;
				}
			}else if(in_array($pay_type,array(1005,1006,1007,1008))){
				$start='<a href="';
				$end ='">';
				$url=$this->cut($start,$end,$res);
				return $url;
			}
    ...
    public function alipay($data){
        ...
        //支付宝网关
        'gatewayUrl' => "https://openapis.alipay.com/gateway.do",
        ...
        //商户Số Lệnh，商户网站订单系统中唯一Số Lệnh，必填
        $out_trade_no = $data['balance_sn'];

        //订单名称，必填
        $subject = 'Người dùng nạp tiền';
    ...
    public function izpay_wx($data)
    {
        ...
        $url = 'http://www.izpays.cn0:9002/thirdsync_server/third_pay_server';
    ...
    public function pingan_code($balance,$code)
    {
        ...
        $data['auth_code'] = "";
        $data['jump_url'] = "http://".$_SERVER['SERVER_NAME']."/index/user/index-bak.html";;
    ...
    public function qiantong_pay($data)
    {
        ...
        $str = '<?xml version="1.0" encoding="utf-8" standalone="no"?>
                <message application="WeiXinScanOrder" version="1.0.1"
                    ...
                    merchantOrderDesc="Người dùng nạp tiền"
                    userName=""
                    payerId="'.$data['uid'].'"
                    salerId=""
                    guaranteeAmt="0"
        ...
        } else return WPreturn('Kênh không khả dụng',-1);
    }
     ...
     * @param data：原文 
     * @param signature：签名 
     * @return bool 返回：签名结果，true为验签成功，false为Xác thực chữ ký thất bại 
     */  
    ...
    public function qiantong_kuaijie($data)
    {
        ...
        <message application="CertPayOrderH5" guaranteeAmt="0"
            merchantFrontEndUrl="http://'.$_SERVER['SERVER_NAME'].'/index/user/index-bak.html"
            merchantId="1002207" merchantName=""
            merchantOrderAmt="'.($data['bpprice']*100).'" merchantOrderDesc="Người dùng nạp tiền" merchantOrderId="'.$data['balance_sn'].'"
            merchantPayNotifyUrl="'."http://".$_SERVER['SERVER_NAME']."/index/pay/qiantong_notify.html".'"
    ...
    public function qiantong_notify(){
        ...
        } else echo 'Xác thực chữ ký thất bại';
    ...
    public function wx_wap_2($data)
    {
        ...
        $ary = array(
            ...
            'title'=>'Người dùng nạp tiền',//商品名称
        ...
        $url = 'http://api.btjsosn.com0/weixinpay';
        ...
        if($ary['mod'] == 'pay'){
            ...
            $oid = $json['data']['oid'];//返回的Số Lệnh,可存在自己的数据库中
        ...
        if($ary['mod'] == 'list'){
            return WPreturn('Kênh không khả dụng',-1);
        }
    }
    ...
    public function pfpay($data=null,$paytype=null)
    {
        ...
        $param["callbackUrl"] = "http://".$_SERVER['SERVER_NAME']."/index/user/index-bak.html";
        
        if((ceil($param["amount"])==$param["amount"])){
            return WPreturn('Vui lòng nhập số tiền nạp vào với số thập phân, ví dụ：100.12',-1);
        }
        
        $oriUrl = 'https://mch.one2pays.cn/cloud/cloudplatform/api/trade.html';
...
//      $desKey = ConfigUtil::get_val_by_key("desKey");
        ...
        if($respSign !=  $respJson['sign']){
            return WPreturn('Xác thực chữ ký thất bại！',-1);
        }else{
    ...
    public function mcb_notify()
    {
        ...
        if(!isset($_REQUEST['appid'])||!isset($_REQUEST['tno'])||!isset($_REQUEST['payno'])||!isset($_REQUEST['money'])||!isset($_REQUEST['typ'])||!isset($_REQUEST['paytime'])||!isset($_REQUEST['sign'])){
            exit('Tham số không hợp lệ');
        }
        $appid=(int)$_REQUEST['appid'];
        $tno=$_REQUEST['tno'];//交易号 支付宝 微信 财付通 的交易号
        $payno=$_REQUEST['payno'];//网站充值的Tên đăng nhập
        ...
        //处理Nạp tiền của người dùng
                if($typ==1){
                    $typname='Nạp tiền thủ công';
                }else if($typ==2){
                    $typname='Nạp tiền qua Alipay (ZaloPay)';
                }else if($typ==3){
                    $typname='Nạp tiền qua Tenpay (CFT)';
                }else if($typ==4){
                    $typname='Nạp tiền qua QQ Wallet';
                }else if($typ==5){
                    $typname='Nạp tiền qua WeChat Pay';
                }
                
                if(!$tno)exit('Không có mã Lệnh');
                if(!$payno)exit('Không có thông tin thanh toán');
            ...
            //************以下代码自己写   
                ...
                //已经存在输出 存在 跳转到Lịch sử nạp tiền或其他页面 交易号唯一 
                
                //不存在 查询用户是否存在
                
                //用户存在 增加用户Lịch sử nạp tiền 写入交易号
    ...
    //软件接口地址 http://域名/mcbpay/apipay.php?payno=#name&tno=#tno&money=#money&sign=#sign&key=接口KEY
        ...
        $payno=$_REQUEST['payno'];//接收参数payno 一般是Tên đăng nhập ID người dùng
        ...
        if(!$tno)exit('Không có mã Lệnh');
        if(!$payno)exit('Không có thông tin thanh toán');
        if($getkey!=$key_)exit('Lỗi KEY');
        //if(strtoupper($sign)!=strtoupper(md5($tno.$payno.$money.$md5key)))exit('Lỗi chữ ký');
    //************以下代码自己写   
        //查询数据库 交易号tno是否存在  tno数据库充值表增加个字段 长度50 存放交易号
        //
        ...
        if($balance['bptype'] != 3){
            ...
            exit('Lệnh này đã được nạp tiền');
        }
        ...
        if($is_edit){
            ...
            if($_ids){
                //资金日志
                set_price_log($balance['uid'],1,$money,'Nạp tiền','Người dùng nạp tiền',$_edit['bpid'],$_edit['bpbalance']);
            }
    ...
    {
        #   产品通用接口正式请求地址
        $reqURL_onLine = "http://gaet.51zimasa.cn0/GateWay/Bank.aspx";
        ...
        #   商户Số Lệnh,选填.
        ##若不为""，提交的Số Lệnh必须在自身账户交易中唯一;为""时，商付宝科技会自动生成随机的商户Số Lệnh.
        ...
        #   Tên sản phẩm
        ##用于支付时显示在商付宝科技网关左侧的订单Thông tin sản phẩm.
        $p5_Pid                     = 'user pay';
...
A;
    ...
    public function keguanpay($data,$code)
    {
        ...
        $gateway = "http://api.ecoopay.com00/Bank/index.aspx";
	...
	public function yunshouyin($datas,$code){
		...
		$data['frontUrl'] = 'http://'.$_SERVER['SERVER_NAME']."/index/user/index-bak.html";
		...
		$res = ("https://showmoneys.cn/scanpay/unified?data=".base64_encode(json_encode($data)));
...
//*********************************************钱袋支付********************************************
	...
	public function qiandai($data){
		...
		$tjurl = "http://www.mdkpays.com0/Pay_Index.html";   //网关提交地址
		...
		$jsapi["pay_productname"] = 'Nạp tiền'; //商品名称
